...
public class DeleteFavoriteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,HttpServletResponse response) 
		...
		try {
			...
			if(del==1)
			{
				pstmt = con.prepareStatement("delete from favoriteinfo  where userid=?");
				pstmt.setInt(1, user.getId());
				pstmt.executeUpdate();
			}else {
				pstmt = con.prepareStatement("delete from favoriteinfo  where userid=? and bookid=?");
				pstmt.setInt(1, user.getId());
				pstmt.setString(2, request.getParameter("bookid"));
				pstmt.executeUpdate();
			}
			result = "admin/users/userfavorite.jsp";
		} catch (Exception e) {
			e.printStackTrace();
			result = "error.jsp";
		} finally {
